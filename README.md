...
**The poll is read-only by construction.** It runs with an allowlist limited to read verbs (`gh pr list/view/diff`, `gh issue list/view`, the read-only preflights `gh auth status`/`gh --version`, and the Slack backend's `read`/`thread` — `spy` or `scripts/slack-mcp.sh` depending on `agent.slack.method`), so it can find work and draft responses but cannot post anything. The Slack `send` verb is never allowlisted in the poll under either method. Every outbound action stays behind the approval gate.
