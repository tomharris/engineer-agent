...
# Routing Tier 3b as a typed Choice over the config-derived candidate set. Inert unless
# agent.typesafe.routing.enabled is true AND a credential (plus curl and jq) resolves — see
# rt_judge_enabled. Sourcing these costs nothing and opens nothing.
...
if rt_judge_enabled; then
  JUDGE_ROUTE=1
  log "poll-github-prs: routing Tier 3b judged inline (min_confidence=$(rt_judge_min))"
fi
...
while IFS= read -r full; do
  ...
  while IFS="$TAB" read -r num title_b64 author url labels_raw head base changed reviewers_raw body_b64; do
    ...
    #
    # Safe to compute before the kind is settled because queue_lookup is FAMILY-WIDE: `pr-review`
    ...
    pre_disp="$(queue_disposition pr-review "$sid")"
    ...
    # compare the FINAL slug, and the reviewer filter below it reads the routed project's
    # github.review_requested_for.
    ...
    if [ "$DRY_RUN" -eq 0 ]; then
      write_pr_item --path "$target" --source-url "$url" --source-id "$sid" --title "$title" \
        ...
        --routing-method "$rmethod" --routing-rationale "$rrat" --matched "$matched"
      printf '%s\n' "$target" >> "$WRITTEN"
    fi
