...
if rt_judge_enabled; then
  JUDGE_ROUTE=1
  log "poll-slite: routing Tier 3b judged inline (min_confidence=$(rt_judge_min))"
fi
...
while IFS= read -r label; do
  ...
  while IFS= read -r hit || [ -n "$hit" ]; do
    ...
    #
    # Safe to compute before the kind is settled because queue_lookup is FAMILY-WIDE: `doc-review`
    ...
    pre_disp="$(queue_disposition doc-review "$sid")"
    ...
    if [ "$needs_route" = "1" ] && [ "$JUDGE_ROUTE" -eq 1 ] && [ "$needs_paid_judgment" -eq 1 ]; then
      judge_out="$(rt_judge_route "$title" "$TMPD/body" slite "$matched")"
